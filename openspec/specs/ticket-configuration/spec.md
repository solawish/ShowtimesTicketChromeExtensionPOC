...
- **AND** 票種下拉選單變為啟用狀態
- **AND** 票種選單載入 `payload.ticketTypes[]` 中的票種選項，按照 `ticketType.sortOrder` 欄位進行升序排序（使用 `ticketType.title` 作為顯示，`ticketType.category.subCategory` 作為值）
- **AND** 若票種缺少 `sortOrder` 欄位或 `sortOrder` 為 null/undefined，則將該票種排在已排序票種之後
- **AND** 數量選單保持啟用狀態（不受影響）
- **AND** 所有下層選單的內部狀態（config 值）被重置為 null 或預設值（數量除外）
- **AND** 載入完成後隱藏載入提示
...
- **AND** 更新票種下拉選單的選項，按照 `ticketType.sortOrder` 欄位進行升序排序
- **AND** 若票種缺少 `sortOrder` 欄位或 `sortOrder` 為 null/undefined，則將該票種排在已排序票種之後
